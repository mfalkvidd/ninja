...
class ORMTypeString implements ORMTypeI {
	...
	public function generate_set ($context) {
		...
		$context->write("\$value = (string)\$value;");
	...
	public function generate_iterator_set ($context) {
		...
		$context->write("\$obj->{$this->name} = (string)\$values[\$prefix.'{$this->backend_name}'];");
		$context->write("}");
	}

	public function generate_array_set ($context) {
		$context->write("if(array_key_exists('{$this->name}', \$values)) {");
		$context->write("\$obj->{$this->name} = (string)\$values['{$this->name}'];");
		$context->write("}");
	}
